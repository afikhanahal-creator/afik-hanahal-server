...
// ── Admin WhatsApp alert — sent immediately when a new lead arrives ───────────
// Target: WA_ADMIN_PHONE env var (default: 972559811814 — Afik's number)
const ADMIN_WA_PHONE = process.env.WA_ADMIN_PHONE || '972559811814'

export async function sendAdminWhatsApp(lead) {
  if (!GREEN_INSTANCE || !GREEN_TOKEN) {
    console.warn('[WA admin] skipping — Green API not configured')
    return
  }
  ...
  const lines = [
    ...
    `📞 *טלפון:* ${lead.phone || '—'}`,
    `📧 *אימייל:* ${lead.email || '—'}`,
    lead.propTitle    ? `🏡 *נכס:* ${lead.propTitle}${lead.propLocation ? ' — ' + lead.propLocation : ''}` : null,
    lead.msg          ? `💬 *הודעה:* ${lead.msg}` : null,
    lead.source && lead.source !== 'website' ? `🌐 *מקור:* ${lead.source}` : null,
  ].filter(Boolean).join('\n')

  const url = `${GREEN_BASE_URL}/waInstance${GREEN_INSTANCE}/sendMessage/${GREEN_TOKEN}`
  console.log(`[WA admin] → ${ADMIN_WA_PHONE}`)
  try {
    const resp = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ chatId: `${ADMIN_WA_PHONE}@c.us`, message: lines }),
      signal: AbortSignal.timeout(15000),
    })
    const text = await resp.text().catch(() => '')
    if (resp.ok) {
      console.log(`[WA admin] ✓ alert sent to ${ADMIN_WA_PHONE}`)
    } else {
      console.error(`[WA admin] Green API ${resp.status}: ${text.slice(0, 200)}`)
    }
  } catch (e) {
...
// ── Generic admin WhatsApp notice ─────────────────────────────────────────────
// Sends an arbitrary text message to the admin number (used for system alerts
// such as Supabase storage approaching the free-tier limit). Returns true on
// success, false otherwise — never throws.
export async function sendAdminWhatsAppText(message) {
  if (!GREEN_INSTANCE || !GREEN_TOKEN) {
    console.warn('[WA admin] skipping notice — Green API not configured')
    return false
  }
  const url = `${GREEN_BASE_URL}/waInstance${GREEN_INSTANCE}/sendMessage/${GREEN_TOKEN}`
  try {
    const resp = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ chatId: `${ADMIN_WA_PHONE}@c.us`, message }),
      signal: AbortSignal.timeout(15000),
    })
    const text = await resp.text().catch(() => '')
    if (resp.ok) {
      console.log(`[WA admin] ✓ notice sent to ${ADMIN_WA_PHONE}`)
      return true
    }
    console.error(`[WA admin] notice Green API ${resp.status}: ${text.slice(0, 200)}`)
    return false
  } catch (e) {
    console.error('[WA admin] notice failed:', e.message)
    return false
  }
}
