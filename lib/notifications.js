...
export async function sendWAFollowUp(lead) {
  if (!GREEN_INSTANCE || !GREEN_TOKEN) {
    console.warn('[WA followup] WA_GREENAPI_INSTANCE or WA_GREENAPI_TOKEN not set — skipping')
    return
  }
  if (!lead.phone) return

  const to = toIntlPhone(lead.phone)
  if (!to) { console.warn('[WA followup] could not parse phone:', lead.phone); return }

  const message = FOLLOWUP_TEMPLATE.replace(/\{name\}/g, lead.name || '')
  ...
  console.log(`[WA followup] sent to ${to}`)
}
