...
// Delete every Storage object referenced by `data` that is NOT referenced by
// `keepData`. Used on property delete (keepData = null) and on edit (keepData =
// the new version) so removed images are cleaned up.
export async function cleanupOrphans(data, keepData = null) {
  const remove = extractStorageObjects(data)
  if (!remove.length) return
  const keep = new Set(extractStorageObjects(keepData).map(o => `${o.bucket}::${o.path}`))
  const toRemove = remove.filter(o => !keep.has(`${o.bucket}::${o.path}`))
